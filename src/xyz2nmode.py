...
class prob_dist_on_nm:
      """
         This class projects a whole trajectory into normal modes. 
         This can be done either with serial or parallel execution
         Args:
         phonon_info_file = path of the ipi-restart file of the phonon calc or 
                            phonon file generated by pyepfd
         ref_struc = path for geometry optimized xyz file
         traj_file = Actual MD/PIMD trajectory
         prefix = Out result prefix
         start_frame = starting frame of the trajectory, default 1st frame
         end_frame = end frame of the trajectory, default last frame
         par_exec = If true parallel execution would be preferred provided MPI4PY available
         mode_list = Supplied mode on which projection must be made, default all active modes based on
                     acoustic sum rule in the restart file of the phonon calculation
         freq_scaled = Output's in frequency scaled unit or not. If frequency scaled then correspondig 
                       result would be equivalent to V(x) = 0.5*x**2 
         nmode_traj = If True it would write the whole trajectory in normal mode, 
         hist_start = Grid points from where histogram of probability density would start.
         hist_end = Grid points from where histogram of probability density would end.
